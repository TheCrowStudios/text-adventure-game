...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const displayItemInformation = (item: any) => {
        ...
        if (textItemInformation) {
            ...
            if (item.useable) {
                const btnUse = document.createElement('button') as HTMLButtonElement;
    ...
    try {
        if (gameDataFromDb) game = new GameEngine(JSON.parse(gameDataFromDb), appendOutput, invSize);
        else game = new GameEngine(gameData, appendOutput, invSize);
    } catch (error) {
