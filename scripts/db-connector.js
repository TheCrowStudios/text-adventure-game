...
export async function saveGameState(username, slot, state) {
...
export async function getSlotModifiedAtFromPosition(username, slot) {
    const user = await getUserByUsername(username);
    if (user !== null && user.data && user.data.length > 0) {
...
export async function getGameStateFromSlot(username, slot) {
    const user = await getUserByUsername(username);
    if (user !== null && user.data && user.data.length > 0) {
